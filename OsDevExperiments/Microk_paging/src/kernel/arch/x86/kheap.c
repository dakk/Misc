...
uint32_t kmalloc_int(uint32_t sz, int align, uint32_t *phys)
{
    if (kheap != 0)
    {
        void *addr = phys_alloc(sz, (uint8_t)align, kheap);
        if (phys != 0)
        {
...
void *phys_alloc(uint32_t size, uint8_t page_align, heap_t *heap)
{
    ...
    if (iterator == -1) // If we didn't find a suitable hole
    {
        ...
        return phys_alloc(size, page_align, heap);
    }
...
void phys_free(void *p, heap_t *heap)
{
