			...
			{	
				...
				for(j = 0; j <= size; j++)
				{
					if(j == size)
					{
						...
						/* Check which process is allocating */
						if(task_current == NULL)
							phys_allocated[ff].pid = -1;
						else
							phys_allocated[ff].pid = task_current->id;
						return (void *) adr;
					}
...
/** Deallocated all blocks of a process */
void mm_free_by_pid(pid_t pid)
{
	unsigned i;
	
	for(i = 0; i < PHYS_MAX_BLOCKS; i++)
		if(phys_allocated[i].pid == pid)
		{
			if(phys_allocated[i].dim != 0)
				phys_free(phys_allocated[i].address);
		}	
}


uint32_t mm_get_used_mem_by_pid(pid_t pid)
{
	unsigned i;
	uint32_t data = 0;
	...
	for(i = 0; i < PHYS_MAX_BLOCKS; i++)
		if(phys_allocated[i].pid == pid)
			data += phys_allocated[i].dim;
		
	return data * PHYS_PAGE_SIZE;
}
