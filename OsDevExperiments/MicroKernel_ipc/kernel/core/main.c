...
void test_ipc()
{
	...
	while(1)
	{
		...
		{
			if(msg_recv(&m))
				kprintf("received message from %d to %d\n", m.from, m.to);
		}
		else
		{
			m.to = 1;
			m.from = sched_get_pid();
			msg_send(&m);
...
void kmain(void *multiboot)
{
	...
	ker_symbols[KSI_MSG_SEND] =	(ker_entry) msg_send;
	ker_symbols[KSI_MSG_RECV] =	(ker_entry) msg_recv;
	ker_symbols[KSI_MSG_CLEAR] =	(ker_entry) msg_clear_queue;
	...
	/*
	sched_exec("test1", test_ipc, NULL, NULL);
	sched_exec("test2", test_ipc, NULL, NULL);
